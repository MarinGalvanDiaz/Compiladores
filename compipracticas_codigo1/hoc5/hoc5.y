...
stmt:     expr   { code(pop1); }
	...
	| while cond stmt end {
		...
		($1)[2] = (Inst)$4; }   /* terminar si la condici�n no se cumple */
   
	| if cond stmt end {    /* proposici�n if que no emplea else */ 
		($1)[1] = (Inst)$3;     /* parte then */ 
		($1)[3] = (Inst)$4; }   /* terminar si la condici�n no se cumple */ 
       
	| if cond stmt end ELSE stmt end {  /* proposici�n if con parte else */
		($1)[1]   =   (Inst)$3;	/*  parte then  */
		($1)[2]   =   (Inst)$6;	/* parte else   */
		($1)[3]   =   (Inst)$7;   } /*   terminar si la condici�n no se cumple  */
	|   '{'   stmtlist   '}' {   $$  =  $2;   }
        ;
cond:	'('   expr   ')'      {   code(STOP);  $$  =  $2;   }
;
                  //$$ = code(whilecode); code(STOP);  code(STOP);
