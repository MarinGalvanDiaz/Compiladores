...
Termino *clonaTermino(Termino *t){
   return creaTermino(t->coefi, t->expo);
}
           ...
           for (p = pol->cab; p; p = p->sig) {
			...
			while (act->expo == cont->expo) {
				//act->coefi += cont->coefi;
                                act->coefi = act->coefi+cont->coefi;
				p->sig = p->sig->sig;
				if (p->sig) cont = (Termino *)p->sig->dato;
				else break;
			}
...
Polinomio *restaPolinomio(Polinomio *p, Polinomio *q) {
	...
	for (aux = p->cab; aux; aux = aux->sig) {
	 	tp1 = (Termino *)aux->dato;
                tp1->band = 0;
                //printf("FUERA for INTER <%.4g, %d> ", tp1->coefi,tp1->expo);
		for (aux2 = q->cab;aux2;aux2 = aux2->sig) {
			tp2 = (Termino *)aux2->dato;
			if ((tp1->expo == tp2->expo)) {
         //printf("for <%.4g, %.4g, %d> ", tp1->coefi,tp2->coefi,tp1->expo);
				insertaOrdA(
          (void *)creaTermino(tp1->coefi-tp2->coefi, tp1->expo),
						&resta, cmpTermino);
			    tp1->band = 1;
		...
		if (tp1->band == 0){
           //printf("for IF<%.4g, %d> ", tp1->coefi,tp1->expo);
			insertaOrdA((void *)creaTermino(tp1->coefi,tp1->expo),
					&resta, cmpTermino);
                }
	}
...
Polinomio *moduloPolinomio(Polinomio *p, Polinomio *q) {
   ...
   Termino *uq=ultimoTermino(q);
   int grado=up->expo-uq->expo; 
   if(up->expo == 1 && uq->expo == 0){
      return creaPolinomio(0, 
         creaNodoL(creaTermino((int)up->coefi % (int)uq->coefi, 0), NULL), 1);
   }
   if (up->expo <  uq->expo || (uq->expo == 0 && uq->coefi == 1)) 
      return creaPolinomio(0, creaNodoL(creaTermino(0, 0), NULL), 1);
...
Polinomio *inverso(Polinomio *a, Polinomio *b) {
    Polinomio *quotient, *residuo, *t0, *t1, *t2;
    Termino *up=ultimoTermino(a);
    Termino *uq=ultimoTermino(b);
    Termino *ur;
    t0 = creaPolinomio(0, creaNodoL(creaTermino(0, 0), NULL), 1);
    t1 = creaPolinomio(0, creaNodoL(creaTermino(1, 0), NULL), 1);
    while (!((uq->expo == 0 ) && (uq->coefi == 0)) ){
        Polinomio *r2, *multi;   
        residuo  = moduloPolinomio(a, b);
        ur=ultimoTermino(residuo);
        if(ur->expo == 0 && ur->coefi == 0) break;
        r2 = restaPolinomio( a, residuo);
        simplifica(r2);
        quotient = dividePolinomio(r2, b); 
        puts("DIVI Q----------------------------------------------------");
        imprimePolinomio(quotient);
        puts("DIVI Q----------------------------------------------------");
        //simplifica(quotient);
        t2 = restaPolinomio( t0, multiplicaPolinomio(quotient, t1));
        simplifica(t2);
        puts("RESTA T2----------------------------------------------------");
        imprimePolinomio(t2);
        puts("RESTA T2----------------------------------------------------");
...
Polinomio *dividePolinomio(Polinomio *p, Polinomio *q) {
   ...
   Termino *uq=ultimoTermino(q);
   int grado=up->expo-uq->expo;  
   if(uq->expo == 0 && uq->coefi == 1) /* div entre 1*/
      return p;
   if (up->expo <  uq->expo) 
      return creaPolinomio(0, creaNodoL(creaTermino(0, 0), NULL), 1);
     Termino *tp1=creaTermino(  up->coefi/uq->coefi, up->expo-uq->expo);
