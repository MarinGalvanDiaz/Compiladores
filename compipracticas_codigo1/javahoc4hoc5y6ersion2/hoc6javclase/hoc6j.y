...
list:   
    ...
    | list defn ';'  { 
      int pc=0;
      System.out.println("progsize="+maq.getProg().size());
      for(pc=0; pc < maq.getProg().size(); pc=pc+1){
	  System.out.println("pc="+pc+" inst "+maq.getProg().elementAt(pc));
      }
                        //return 1 ; 
                      }
    | list asgn '\n'  { maq.code("pop"); 
...
int yylex(){
   ...
   if (!st.hasMoreTokens())
      if (!newline){
         System.out.println("nva linea");
	 newline=true; 
         return '\n';
      }
      else {
         System.out.println("yylex 0000000000000000");
	 return 0;
      }	
   s = st.nextToken();
...
public static void main(String s[]){
   ...
   newline=false;
   for(maq.initcode(); (valpar=par.yyparse ()) != 0 ; maq.initcode()){
   ...
   System.out.println("desp for valpar = ("+valpar+")");		
}
